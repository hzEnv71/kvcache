...
func Register(svcName, addr string, stopCh <-chan error) error {
	...
	// 注册服务，使用完整的key路径
	key := fmt.Sprintf("/services/%s/%s", svcName, addr)//service/kv-cache/127.0.0.1:8001
	_, err = cli.Put(context.Background(), key, addr, clientv3.WithLease(lease.ID))//将服务地址写入etcd
	if err != nil {
		cli.Close()
